...
def write_df(results, score_name):
    ...
    df.to_csv(os.path.join(base_path, f'{score_name}.csv'), index=False)
...
for model in argv[2:]:
    for lang_pair in sorted(listdir(os.path.join(base_path, model))):
        extn = lang_pair.split('.')[1]
        
        if extn == "json":
            lang = lang_pair.split('-')[1].split('.')[0]
            with open(os.path.join(base_path, model, lang_pair), 'r') as f:
