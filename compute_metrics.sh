...
if [[ $tgt_lang == 'en' ]]; then
    ...
    parallel --pipe --keep-order python scripts/preprocess_translate.py $tgt_lang < $ref_fname > $ref_fname.tok 
    parallel --pipe --keep-order python scripts/preprocess_translate.py $tgt_lang < $pred_fname > $pred_fname.tok 
